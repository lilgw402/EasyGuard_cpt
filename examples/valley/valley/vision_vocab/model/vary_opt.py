...
from model.vision_encoder.sam import build_sam_vit_b


from transformers import OPTConfig, OPTModel, OPTForCausalLM

from model.plug.transforms import train_transform, test_transform
...
class varyOPTModel(OPTModel):
    ...
    def initialize_vision_modules(
        ...
        # image_processor = CLIPImageProcessor.from_pretrained('/root/EasyGuard/examples/valley/Vary-toy/CLIP_VL')
        # 1024*1024

        if pretrained_stage1_model is not None:
            base_model_state_dict = torch.load(pretrained_stage1_model)
            self.vision_tower.load_state_dict(base_model_state_dict)
            print("loading base vison vocabulary weight from {} ".format(pretrained_stage1_model))
        ...
        if vision_tower is not None and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            for image in images:
                ...
                with torch.set_grad_enabled(True):
                    cnn_feature = vision_tower(image)
