...
import sys
sys.path.append("valley/vision_vocab")
sys.path.append("valley/vision_vocab/data")
import io
import os
...
class CaptionDataset(BaseDataset):
    ...
    def __init__(self, datasets, tokenizer, multimodal_cfg):
        ...
        conversation_lib.default_conversation = conversation_lib.conv_templates["conv_prd_cp"]
        ...
        for name in datasets.split("+"):
            ...
            data = [json.loads(item) for item in temp_data]
        ...
        logging.warning(f"{len(self.list_data_dict)} conversations in total.")
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(data, dict):
            if 'image' in data:
                ...
                try:
                    image_str = download_url_with_exception(image_url)
                    image = Image.open(io.BytesIO(image_str)).convert('RGB')
                    # image = Image.open(image_path + image_file).convert('RGB')
                except:
                    print(image_str)
                    print(f'cannot identify image file {image_str}.')
                    return self.__getitem__(0)

                try:
                    image_high = self.image_processor(image)
                except:
                    print(f'image {image_url} are broken or grayscale! we thus select 0-th sample instead!')
                    return self.__getitem__(0)
